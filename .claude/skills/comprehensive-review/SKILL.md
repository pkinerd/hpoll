...
positives, flawed reasoning, incorrect severity assessments, duplicate issues,
and bad recommendations before they are persisted.

**Strict 1:1 rule: launch exactly ONE sub-agent per new issue, all in parallel**,
using the Agent tool. Each sub-agent receives exactly one issue to review and
must not review any other issues. Do NOT batch multiple issues into a single
sub-agent — each issue requires independent, isolated analysis.

For each new issue, launch a `general-purpose` sub-agent with this task:

> You are reviewing exactly ONE issue. Do not review or comment on any other
> issues.
>
...
> finding is real, check for duplication against existing issues, and assess
> whether the severity and recommendation are appropriate.
>
...
> 4. Check for duplication against existing issues:
>    - Read `/tmp/claude-issues/INDEX.md` to see all existing issues (both
>      pre-existing and newly created in this review)
>    - Read any existing issue files that cover similar topics, areas, or files
>    - If this new issue substantially overlaps with an existing issue, your
>      verdict should be `retract` with reasoning that it duplicates issue #<id>
...
>      - `retract`: the issue is a false positive, not actionable, or duplicates
>        an existing issue — should be removed. If duplicating, specify which issue.
...
#### Constructing the sub-agent calls

1. List all new issue files created in Step 5
2. For EACH file, create exactly ONE Agent tool call with the issue file path
   substituted into the prompt template above
3. Every Agent tool call must reference exactly one issue file path
4. Do NOT combine multiple issues into a single Agent call

**Critical:** Launch all sub-agents in a single message to maximize throughput.
They are fully independent of each other. There must be exactly as many Agent
tool calls as there are new issue files — no more, no fewer.
...
- **Critical review sub-agents are mandatory**: every new issue MUST be reviewed
  ...
  in a single message, one per new issue. This step catches false positives,
  flawed reasoning, and duplicate issues before they are persisted.
- **Strict 1:1 sub-agent mapping in Step 6**: each new issue MUST have its own
  dedicated sub-agent. Never batch multiple issues into a single sub-agent call.
  Batching defeats the purpose of independent critical review.
